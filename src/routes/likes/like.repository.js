...
//-------------------------------------------------------------------------------------------
// export const findTopLikedCommentAuthors = async (userId) => {
//     // 좋아요 데이터를 포함하여 모든 댓글을 조회하고, 각 좋아요에 대한 댓글 작성자의 ID와 닉네임을 추출합니다.
//     const likes = await prisma.likes.findMany({
//         include: {
//             comment: {
//                 include: {
//                     user: { select: { userId: true, nickname: true } }, // 댓글 작성자의 ID와 닉네임을 포함하여 조회
//                 },
//             },
//         },
//     });

//     // 좋아요 받은 각 댓글 작성자 ID 별로 좋아요 수를 계산합니다.
//     const commentAuthorLikesCount = likes.reduce((acc, like) => {
//         const commentAuthorId = like.comment.user.userId; // 댓글 작성자의 ID 추출
//         acc[commentAuthorId] = (acc[commentAuthorId] || 0) + 1; // 해당 ID의 좋아요 수를 누적
//         return acc;
...
//     // 좋아요 수에 따라 사용자를 내림차순으로 정렬합니다.
//     let sortedAuthors = Object.entries(commentAuthorLikesCount)
//         .map(([commentAuthorId, likes]) => ({ commentAuthorId: parseInt(commentAuthorId), likes }))
//         .sort((a, b) => b.likes - a.likes);

//     // 좋아요 수가 가장 많은 상위 5명을 추출합니다.
//     let topFiveAuthors = sortedAuthors.slice(0, 5);

//     // 로그인한 사용자가 항상 목록에 포함되도록 합니다.
//     const userIndex = sortedAuthors.findIndex((author) => author.commentAuthorId === userId);
//     const userLikes = commentAuthorLikesCount[userId] || 0; // 좋아요 수가 없는 경우 기본값은 0입니다.
...
//             likes: userLikes,
//             nickname: loginUser.nickname,
//             rank: userIndex !== -1 ? userIndex + 1 : sortedAuthors.length + 1,
//         });
//     }

//     // 목록을 다시 정렬하여 순위를 업데이트합니다, 특히 사용자가 추가된 경우
//     topFiveAuthors = topFiveAuthors.sort((a, b) => b.likes - a.likes);
//     topFiveAuthors.forEach((author, index) => {
//         author.rank = index + 1; // 순위 할당
//     });
